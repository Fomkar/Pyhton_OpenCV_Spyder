# -*- coding: utf-8 -*-
"""
Created on Fri Oct 27 09:39:29 2023
...
for filename in os.listdir(path):
    if filename.endswith("2_2.bmp"):
        ...
        # image = cv2.medianBlur(gray_image, 9)
        # image = cv2.bilateralFilter(gray_image, 9,75,75)
        
        edges = cv2.Canny(gray_image,50,25)
        ...
        _,threshold = cv2.threshold(edges, 60, 255, cv2.THRESH_BINARY)
        ...
        # erode = cv2.erode(trehsold, (5,5),iterations = 15)
        # dilate = cv2.dilate(threshold, (5,5),iterations = 11)
        # cv2.namedWindow("Dilate_image",cv2.WINDOW_NORMAL)
        # cv2.imshow("Dilate_image",dilate)
...
#cnt = contours[0]
        ...
        centerx = int(((contours_array[2][0]) + (contours_array[2][2] / 2)))
        centery = int(((contours_array[2][1]) + (contours_array[2][3] / 2)))
        radius = int(w/2)
        center_square_x = int(((contours_array[0][0]) + (contours_array[0][2] / 2)))
        y_square = contours_array[0][1]
