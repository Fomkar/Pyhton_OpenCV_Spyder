...
for filename in os.listdir(path):
    if filename.endswith(image_name):
        ...
        _,threshold = cv2.threshold(gray_image, 30, 250, cv2.THRESH_BINARY)
        ...
        dilate = cv2.dilate(threshold, (5,5),iterations = 1)
        cv2.namedWindow("Dilate_image",cv2.WINDOW_NORMAL)
        cv2.imshow("Dilate_image",dilate)
...
for filename in os.listdir(path):
    if filename.endswith(image_name):
        ...
        _,threshold = cv2.threshold(gray_image, 45, 250, cv2.THRESH_BINARY_INV)
        ...
        dilate = cv2.dilate(threshold, (5,5),iterations = 3)
        cv2.namedWindow("Dilate_image",cv2.WINDOW_NORMAL)
        cv2.imshow("Dilate_image",dilate)
